 ...
 *
 * [Important]
 * This plugin is modified by Diving_Fish.
 * Don't change any codes or parameters of this plugin.
 * 
...
DataManager.processRowNotetags1 = function(group) {
  for (var n = 1; n < group.length; n++) {
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(/<(?:DEFAULT COLUMN):[ ]*(\d+(?:\s*,\s*\d+)*)>/i)) {
        obj.defaultColumn = [];
        var array = JSON.parse('[' + RegExp.$1.match(/\d+/g) + ']');
        obj.defaultColumn = obj.defaultColumn.concat(array);
      }
    }
  }
};
...
Game_Battler.prototype.initRowFormation = function() {
    if (this.isActor()) {
      ...
      var columns = this.actor().defaultColumn;
    } else if (this.isEnemy()) {
      var rows = this.enemy().defaultRow;
      var columns = this.enemy().defaultColumn;
    }
    this._row = rows[Math.floor(Math.random() * rows.length)];;
    this._row = this._row.clamp(1, Yanfly.Param.RowMaximum);
    this._column = columns[Math.floor(Math.random() * columns.length)];;
    this._column = this._column.clamp(1, 5);
    this._rowStatesRaw = undefined;
};
...
Game_Battler.prototype.column = function() {
    if (this._column === undefined) this.initRowFormation();
    return this._column;
}
...
Game_Battler.prototype.positionIndex = function() {
  var group = this.friendsUnit();
  if (Yanfly.Param.RowAliveRowIndex && group === $gameTroop) {
    var index = group.positionAliveMembers(this.row(), this.column()).indexOf(this);
  } else {
    var index = group.positionMembers(this.row(), this.column()).indexOf(this);
  }
  return index;
};
...
// Override
Game_Battler.prototype.setPosition = function(rowId, columnId) {
  if (this._row === undefined || this._column === undefined) this.initRowFormation();
  var currentRow = this._row;
  var currentColumn = this._column;
  this._row = rowId.clamp(1, 5);
  this._column = columnId.clamp(1, 5);
  var changed = currentRow !== this._row || currentColumn !== this._column;
  if (changed) this.friendsUnit().clearBattleRowCache();
  if ($gameParty.inBattle() && changed) BattleManager.requestRefreshRows();
}
...
Game_Unit.prototype.positionSize = function(rowId) {
  return this.positionMembers(rowId).length;
}
...
Game_Unit.prototype.columnAliveMembers = function(columnId) {
    var group = [];
    var length = this.aliveMembers().length;
    for (var i = 0; i < length; ++i) {
      var member = this.aliveMembers()[i];
      if (!member) continue;
      if (member._column === columnId) group.push(member);
    }
    return group;
}

Game_Unit.prototype.positionMembers = function(rowId, columnId) {
  var group = [];
  var length = this.members().length;
  for (var i = 0; i < length; ++i) {
    var member = this.members()[i];
    if (!member) continue;
    if (member.row() === rowId && member.column() === columnId) group.push(member);
  }
  return group;
}

Game_Unit.prototype.positionAliveMembers = function(rowId, columnId) {
  var group = [];
  var length = this.aliveMembers().length;
  for (var i = 0; i < length; ++i) {
    var member = this.aliveMembers()[i];
...
Sprite_Actor.prototype.alterActorHome = function(index) {
    ...
    var columnId = this._actor._column;
...
Sprite_Enemy.prototype.alterEnemyHome = function(index) {
    ...
    var partySize = $gameTroop.members().length;
    var rowId = this._enemy.row();
    var columnId = this._enemy._column;
    var rowSize = $gameTroop.rowSize(rowId);
    var rowMembers = $gameTroop.rowMembers(rowId);
    var rowIndex = this._enemy.rowIndex();
