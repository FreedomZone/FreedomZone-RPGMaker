...
DataManager.isWholeAction = function(obj) {
  ...
  if (obj.scope === 'COLUMN ROW') return true;
  ...
  if (obj.scope === 'BLOCK') return true;
  return true;
};
...
BattleManager.startAllSelection = function() {
  ...
  if (action.isForMultiple()) {
    ...
    this._customTargetSelectGroup = targets;
  } else if (action.item().scope == 'BLOCK') {
    var targets = this.inputtingAction().makeBlockTargets();
    this._customTargetSelectGroup = targets;
  } else if (action.isForEval()) {
    var targets = this.inputtingAction().makeEvalTargets();
    this._customTargetSelectGroup = targets;
  } else {
...
Game_Action.prototype.makeCustomTargets = function() {
  ...
  } else if (this.item().scope == 'BLOCK') {
    targets = this.makeBlockTargets();
...
Game_Action.prototype.makeBlockTargets = function() {
    var targets = [];
    var unit = this.opponentsUnit();
    var target = unit.smoothTarget(this._targetIndex);
    var length = unit.aliveMembers().length;
    for (var i = 0; i < length; i++) {
      var member = unit.aliveMembers()[i];
      if (!member) continue;
      if (member._row == target._row && member._column == target._column) targets.push(member);
    }
    return targets;
}
...
Game_Action.prototype.makeRowTypeTargets = function() {
    ...
    if (scope === 'ENEMY ROW') {
      ...
      var unit = this.opponentsUnit();
      var target = unit.smoothTarget(this._targetIndex);
      var column = target._column;
      targets = unit.columnAliveMembers(column);
...
Window_Help.prototype.makeCustomTargetText = function(action) {
    ...
    var selected_name = this._battler.name();
    try {
      eval(code);
    } catch (e) {
